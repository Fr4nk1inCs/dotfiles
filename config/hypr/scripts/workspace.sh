...
passivews=$(grep -A 5 "$passivemonitor" "$monitors" | awk 'NR==4 {print $1}' RS='(' FS=')')
...
if [[ $workspace -eq $passivews ]] && [[ $activemonitor != "$passivemonitor" ]]; then
	...
	hyprctl dispatch moveworkspacetomonitor "$workspace $activemonitor" &&
		hyprctl dispatch moveworkspacetomonitor "$passivews $passivemonitor" &&
		hyprctl dispatch workspacesilent "$workspace"
fi
